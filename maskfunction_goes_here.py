import numpy as np
def maskfunction(rho,phi,w0,f,k): #parameters radius,f,k are given as an input in case they are used in the phase mask
    '''
    rho, phi are cilindrical coordinates from the field inciding on the lens    
    w0 is the beam's radius in mm, where the amplitude takes the value of exp(-1)    
    f is the focal distance (given in mm)
    k is the wavenumber (given in nm)
    ...
    gamma=4*10**-5
    rho*=10**6 #pasage to nm, to have same units as k

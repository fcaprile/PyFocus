...
def interface_custom_mask_focus_field_XZ_XY(n_list,d_list,ex_lens,ey_lens,alpha,h,Lambda,z_interface,z_FOV,resolution_z,zp0,resolution_focus,resolution_theta,resolution_phi,FOV_focus,x0=0,y0=0,z0=0,plot_plane='X'):
    ...
    n2=n_list[-1]
    #XY plane: 
    Ex_XY,Ey_XY,Ez_XY=interface_custom_mask_focus_field_XY(n_list,d_list,ex_lens,ey_lens,alpha,h,Lambda,z_interface,zp0,resolution_focus,resolution_theta,resolution_phi,FOV_focus,True,x0,y0)
    ...
    #define positions in which to calculate the field:                
    xmax=FOV_focus/2
    x_values=np.linspace(-xmax+x0,xmax+x0,resolution_focus)*(2**0.5) #on the XZ plane the x axis is sqrt(2) times longer in order to make it analogue to the VPP and no_mask functions

...
def snell(n_1,n_2,th_1):
    ...
    th_2_guess = np.lib.scimath.arcsin(n_1*np.sin(th_1) / n_2)
...
def list_snell(n_list,th_0):
    ...
    angles = np.lib.scimath.arcsin(n_list[0]*np.sin(th_0) / n_list)
...
def coh_tmm(pol, n_list, d_list, th_0, lam_vac):
    ...
    * pol, n_list, d_list, th_0, lam_vac--same as input
    ...
    for i in range(1,num_layers-1):
        matrix1=make_2x2_array(exp(-1j*delta[i]), 0, 0, exp(1j*delta[i]),dtype=complex)
        ri=r_list[i,i+1]
        matrix2=make_2x2_array(1, ri, ri, 1, dtype=complex)
        M_list[i] = (1/t_list[i,i+1]) * np.dot(matrix1,matrix2)
...
#        print(Mtilde)        
    ...
    power_entering=zeros((len(n_list)))
    for i in range(len(n_list)):
        power_entering[i] = power_entering_from_r(
                                pol, r, n_list[i], th_0)
...
#%%
def position(xyz,layer,dist,coh_tmm_data): #FUNCION NUEVA DE GERMAN 11/10/17
    """
    ...
    kx=kz*np.sin(th)/np.cos(th)  #=kz*np.sin(th_0)/np.cos(th_0)
    ky=0
    
    if xyz=='x':
        #amplitude of forward-moving wave is Ef, backwards is Eb   
        Ef = vw[0] * exp(1j * kx * dist)  *np.cos(th)/np.sin(th+(2*pi))
        Eb = vw[1] * exp(-1j * kx * dist) *np.cos(th)/np.sin(th+(2*pi))
    if xyz =='y':
        Ef = vw[0] * exp(1j * ky * dist)
        Eb = vw[1] * exp(-1j * ky * dist)
    if xyz == 'z':        
        #amplitude of forward-moving wave is Ef, backwards is Eb
